...
class Job104 extends JobBase
{
     ...
     * @var string
     */
    private $_api_url = 'https://www.104.com.tw/jobs/search/list';
//    private $_api_url = 'https://www.104.com.tw/i/apis/jobsearch.cfm';
    ...
    private function _convert_job_row_data($row)
    {
        ...
        return [
            'title'                =>  isset($row['jobName']) ? $row['jobName'] : NULL,
            'j_code'               =>  isset($row['jobNo']) ? $row['jobNo'] : NULL,
            ...
            'appear_date'          =>  isset($row['appearDate']) ? $row['appearDate'] : NULL,
...
//            'handicompendium'      =>  isset($row['HANDICOMPENDIUM']) ? $row['HANDICOMPENDIUM'] : NULL,
            'role'                 =>  isset($row['jobRole']) ? $row['jobRole'] : NULL,
//            'role_status'          =>  isset($row['ROLE_STATUS']) ? $row['ROLE_STATUS'] : NULL,
//            's2'                   =>  isset($row['S2']) ? $row['S2'] : NULL,
//            's3'                   =>  isset($row['S3']) ? $row['S3'] : NULL,
            'sal_month_low'        =>  isset($row['salaryLow']) ? $row['salaryLow'] : NULL,
            'sal_month_high'       =>  isset($row['salaryHigh']) ? $row['salaryHigh'] : NULL,
...
//            'major_cat_descript'   =>  isset($row['MAJOR_CAT_DESCRIPT']) ? $row['MAJOR_CAT_DESCRIPT'] : NULL,
            'minbinary_edu'        =>  isset($row['optionEdu']) ? $row['optionEdu'] : NULL,
...
//            'others'               =>  isset($row['OTHERS']) ? $row['OTHERS'] : NULL,
    ...
    public function update($conditions = NULL)
    {
        ...
        // 回傳資料
        $record_count        = $json_data['data']['totalCount'];
        $page_size           = (isset($conditions['pgsz'])) ? $conditions['pgsz'] : 20;
        $page                = $json_data['data']['pageNo'];
...
//        $finish_record_count = $page_size * $page;
    ...
    public function get_jobs($conditions)
    {
        ...
        $c_codes = array_column($job_data, 'C');
        ...
        $pagination = [
            'record_count' => $ApiResult['data']['totalCount'],
            'page_count' => $ApiResult['data']['pageNo'],
            'page' => $ApiResult['data']['pageNo'],
            'total_page' => $ApiResult['data']['totalPage']
        ];
